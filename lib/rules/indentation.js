...
module.exports = {
  ...
  'detect': function (ast, parser) {
    var result = [];
    var mixedWarning = false;

    traverse(ast, parser, function (res, opts) {
      ...
      if (res.detected.length > 1) {
        ...
        result = helpers.addUnique(result, {
          ...
          'line': res.reportNode.start.line,
          'column': res.reportNode.start.column,
      ...
      if (
        res.reportCondition &&
        !mixedWarning &&
        res.spaceLength / opts.lintSize !== res.level
      ) {
        ...
        result = helpers.addUnique(result, {
          ...
          'line': res.reportNode.start.line,
          'column': res.reportNode.start.column,
          'message': 'Expected indentation of ' + res.level * opts.lintSize + ' ' + opts.lintType + opts.plural + ' but found ' + res.spaceLength + '.',
          'severity': parser.severity
        });
      }
      mixedWarning = false;
...
module.exports.fix = function (ast, parser) {
  ...
  traverse(ast, parser, function (res, opts) {
    ...
    var sp = Array.apply(null, new Array(opts.lintSize))
    .reduce(function (memo) {
      return memo + (opts.lintType === 'tab' ? '\t' : ' ');
    }, '');

    if (res.detected.length > 1) {
      res.node.content = res.node.content.replace(/\t/g, sp);
      mixedWarning = true;
    }
    if (
      res.reportCondition &&
      ...
      res.node.content = '\n' + Array.apply(null, new Array(res.level))
      .reduce(function (memo) {
        return memo + sp;
      }, '');
