...
module.exports.fix = function (ast, parser) {
  ...
  ast.traverseByType('ruleset', function (node, j, p) {
    ...
    if (syntax === 'scss') {
      ...
      if (space) {
        if (parser.options.include && !space.space && j !== 1) {
          console.log(space.previous);
          space.previous.content = space.previous.content
          .replace(/\n/g, '\n\n');
        }
        else if (!parser.options.include && space.space) {
          console.log('TBD: implement fix empty-line-between-blocks: false');
        }
      }
    }
    else if (syntax === 'sass') {
      console.log('TBD: implement fix for SASS');
    }
    return true;
  });
};
