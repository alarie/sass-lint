...
helpers.createNode = function (ast, options) {
  return new ast.constructor(Object.assign(options, {
    syntax: ast.syntax
  }));
};

helpers.addChild = function (target, newNodeOptions, index) {
  index = index || target.content.length;
  if (Array.isArray(target.content)) {
    target.insert(
      target.content.length,
      helpers.createNode(target, newNodeOptions)
    );
  }
  else {
    if (typeof newNodeOptions.content === 'string') {
      if (index) {
        target.content += newNodeOptions.content;
      }
      else {
        target.content = newNodeOptions.content + target.content;
      }
    }
